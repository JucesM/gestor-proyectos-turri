...
const ProfileEdit = ({ user, onUpdate, onClose }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Si es un usuario nuevo, primero sincronizar datos de OpenProject
      if (isNewUser) {
        console.log('Syncing user data with OpenProject before avatar upload...');
        const syncResponse = await fetch(`${API_BASE_URL}/users/sync-openproject`, {
          method: 'POST',
          headers: {
      ...
      if (isNewUser) {
        // Crear usuario nuevo con avatar
        const userData = {
          ...formData,
          openproject_id: user.openproject_id
        };
        ...
        if (!createResponse.ok) {
          ...
          throw new Error(`Error al crear el usuario: ${errorData.message}`);
        }
        ...
        // Ahora subir el avatar para el usuario creado
        ...
        if (uploadResponse.ok) {
          ...
          newUser.avatar_url = uploadResult.avatarUrl;
        }

        toast.success('Usuario creado correctamente');
        onUpdate && onUpdate(newUser);
      } else {
        // Solo actualizar avatar para usuario existente
        const formDataUpload = new FormData();
        formDataUpload.append('avatar', selectedFile);
