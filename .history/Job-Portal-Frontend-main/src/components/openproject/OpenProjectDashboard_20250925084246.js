...
const OpenProjectDashboard = () => {
  ...
  const loadMemberAvatars = async (members) => {
    ...
    for (const member of members) {
      if (member.userId) {
        try {
          const response = await fetch(`${API_BASE_URL}/users/by/${member.userId}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
          ...
          if (response.ok) {
            ...
            if (data.user.avatar_url) {
              avatars[member.userId] = data.user.avatar_url;
            }
          }
        } catch (error) {
          console.error(`Error loading avatar for userId ${member.userId}:`, error);
        }
      }
    }
