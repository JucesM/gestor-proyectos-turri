        ...
        <div>
          ...
          <div style={{
            ...
            {members
              ...
              .slice(0, 5)
              .map((member, index) => {
              // Filtrar solo tareas "En curso" asignadas a este miembro
              const memberTasks = tasks.filter(task => {
                const assignee = task._links?.assignee;
                const isAssigned = assignee && assignee.title === member.user;

                // Solo incluir tareas específicamente "En curso"
                const isInProgress = task._links?.status?.title === 'En curso' ||
                                   task._links?.status?.title === 'In Progress' ||
                                   task._links?.status?.title === 'En proceso' ||
                                   task._links?.status?.title === 'In Process' ||
                                   task._links?.status?.title === 'Nuevo' ||
                                   task._links?.status?.title === 'New' ||
                ...
                const shouldInclude = isAssigned && isInProgress;
                ...
                if (isAssigned) {
                  ...
                  console.log(`Is In Progress: ${isInProgress}, Should Include: ${shouldInclude}`);
                }
