          ...
          <div style={{
            ...
            {members
              ...
              const memberTasks = tasks.filter(task => {
                ...
                // Comparación más robusta (ignorar mayúsculas y espacios)
                const isAssigned = assignee &&
                  assignee.title &&
                  member.user &&
                  assignee.title.toLowerCase().trim() === member.user.toLowerCase().trim();

                // Solo incluir tareas específicamente "En curso" (ID 7)
                const isInProgress = task._links?.status?.href === '/api/v3/statuses/7' ||
                                   task._links?.status?.title === 'En curso';

                // Debug logging
                if (isAssigned) {
                  console.log(`=== TASK DEBUG for ${member.user} ===`);
                  console.log(`Task ID: ${task.id}, Subject: ${task.subject}`);
                  console.log(`Assignee: ${assignee?.title}`);
                  console.log(`Status href: ${task._links?.status?.href}`);
                  console.log(`Status title: ${task._links?.status?.title}`);
                  console.log(`Is Assigned: ${isAssigned}, Is In Progress: ${isInProgress}`);
                  console.log(`Should Include: ${isAssigned && isInProgress}`);
                }

                return isAssigned && isInProgress;
              });

              console.log(`=== MEMBER ${member.user} SUMMARY ===`);
              console.log(`Member roles: ${member.roles?.join(', ')}`);
              console.log(`Tasks found: ${memberTasks.length}`);
              memberTasks.forEach(task => {
                console.log(`- Task #${task.id}: ${task.subject} (Status: ${task._links?.status?.title})`);
              });
