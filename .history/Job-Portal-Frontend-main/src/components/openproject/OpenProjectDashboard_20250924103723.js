        ...
        <div>
          ...
          <div style={{
            ...
            {members
              ...
              .slice(0, 5)
              .map((member, index) => {
              // Filtrar tareas activas asignadas a este miembro (excluir solo las cerradas/terminadas)
              const memberTasks = tasks.filter(task => {
                const assignee = task._links?.assignee;
                const isAssigned = assignee && assignee.title === member.user;

                // Excluir solo tareas que definitivamente están cerradas o terminadas
                const isClosed = task._links?.status?.title === 'Cerrado' ||
                                task._links?.status?.title === 'Closed' ||
                                task._links?.status?.title === 'Terminado' ||
                                task._links?.status?.title === 'Completado' ||
                                task._links?.status?.title === 'Done' ||
                                task.status === 'Cerrado' ||
                                task.status === 'Closed' ||
                ...
                const shouldInclude = isAssigned && !isClosed;

                // Debug logging para este miembro
                if (isAssigned) {
                  console.log(`=== TASK FILTER DEBUG for ${member.user} ===`);
                  console.log(`Task ID: ${task.id}, Subject: ${task.subject}`);
                  console.log(`Status (links): ${task._links?.status?.title}`);
                  console.log(`Status (direct): ${task.status}`);
                  console.log(`Is Closed: ${isClosed}, Should Include: ${shouldInclude}`);
                }

                return shouldInclude;
              });

              console.log(`=== MEMBER ${member.user} TASKS SUMMARY ===`);
              console.log(`Total tasks assigned: ${memberTasks.length}`);
              memberTasks.forEach(task => {
                console.log(`- Task #${task.id}: ${task.subject} (Status: ${task._links?.status?.title || task.status})`);
              });

              const currentTask = memberTasks[0];
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      {currentTask?.startDate && currentTask?.dueDate
                        ? `${new Date(currentTask.startDate).toLocaleDateString('es-ES')} – ${new Date(currentTask.dueDate).toLocaleDateString('es-ES')}`
                        : currentTask?.dueDate
