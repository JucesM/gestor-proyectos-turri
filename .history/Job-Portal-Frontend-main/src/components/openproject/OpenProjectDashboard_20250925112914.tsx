...
const OpenProjectDashboard: React.FC = () => {
  ...
  return (
    <div className="bg-white min-h-screen">
      {/* Header */}
      <header className="h-22 bg-blue-900 flex items-center justify-between px-8 text-white">
        <div className="font-pacifico text-xl font-normal cursor-pointer" onClick={() => navigate('/projects')}>
          Gestión de proyectos
        </div>

        <nav className="flex gap-8">
          <span className="relative font-semibold cursor-pointer" onClick={() => navigate('/projects')}>
            Proyectos
            <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-15 h-0.5 bg-yellow-500"></div>
          </span>
          <span className="cursor-pointer opacity-70">Colaboradores</span>
          <span className="cursor-pointer opacity-70">Disponible</span>
        </nav>

        <div className="relative flex items-center gap-3">
          <div
            className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-base font-bold text-blue-900 cursor-pointer"
          ...
          />
          {showDropdown && (
            <div className="absolute top-full right-0 bg-white rounded-lg shadow-lg p-2 min-w-37.5 z-50">
              <button
                onClick={() => { handleLogout(); setShowDropdown(false); }}
                className="bg-none border-none text-black cursor-pointer text-sm p-2 w-full text-left flex items-center gap-2 hover:bg-gray-50"
              >
                <FiLogOut />
                Cerrar sesión
              </button>
      ...
      {/* Main Content */}
      <main className="p-8 grid grid-cols-[1fr_400px] gap-18 max-w-screen-2xl mx-auto">
        {/* Left Column */}
        <div>
          <h1 className="text-5xl font-extrabold text-black mb-8 font-inter">
