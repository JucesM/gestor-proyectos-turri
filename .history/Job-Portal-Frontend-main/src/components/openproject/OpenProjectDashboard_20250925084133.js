...
const OpenProjectDashboard = () => {
  ...
  const [activityAvatars, setActivityAvatars] = useState({});
  ...
  // Cargar avatares de miembros
  const loadMemberAvatars = async (members) => {
    ...
    for (const member of members) {
      if (member.email) {
        try {
          const response = await fetch(`${API_BASE_URL}/users/by/${encodeURIComponent(member.email)}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
          ...
          if (response.ok) {
            ...
            if (data.user.avatar_url) {
              avatars[member.email] = data.user.avatar_url;
            }
          }
        } catch (error) {
          console.error(`Error loading avatar for ${member.email}:`, error);
        }
      }
    }
  ...
  // Cargar avatares para actividad reciente
  const loadActivityAvatars = async () => {
    setLoading(prev => ({ ...prev, activityAvatars: true }));
    const avatars = {};
    const token = sessionStorage.getItem('authToken');

    // Nombres de la actividad reciente
    const activityNames = ["Nicolás", "Jhon", "Angie", "Julio"];

    for (const name of activityNames) {
      try {
        const response = await fetch(`${API_BASE_URL}/users/by/${encodeURIComponent(name)}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
        ...
        if (response.ok) {
          const data = await response.json();
          if (data.user.avatar_url) {
            avatars[name] = data.user.avatar_url;
          }
        }
      } catch (error) {
    ...
    setActivityAvatars(avatars);
    setLoading(prev => ({ ...prev, activityAvatars: false }));
  };
  ...
  const handleEditMember = async (member) => {
    ...
    try {
      ...
      const token = sessionStorage.getItem('authToken');
      const response = await fetch(`${API_BASE_URL}/users/by-openproject/${member.userId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.ok) {
        ...
        const basicUser = {
          openproject_id: member.userId, // Usar userId, no member.id
          name: member.user || '',
          email: member.email || '',
          roles: member.roles || []
        };
              ...
              return (
                <div key={member.id} style={{
                  ...
                  <div style={{
                    ...
                    {memberAvatars[member.email] ? (
                      <img
                        src={memberAvatars[member.email]}
                ...
                {[
                  ...
                  <div key={index} style={{
                    ...
                    <div style={{
                      ...
                      {activityAvatars[activity.split(' · ')[0]] ? (
                        <img
                          src={activityAvatars[activity.split(' · ')[0]]}
                          alt={`${activity.split(' · ')[0]} avatar`}
                          style={{
                            width: '100%',
                            height: '100%',
                            objectFit: 'cover'
