...
/** Obtener detalles de usuario incluyendo avatar */
export async function getUserDetails(userId) {
  const { data } = await op.get(`/users/${userId}`);
  return {
    id: data.id,
    name: data.name,
    email: data.email,
    avatar: data.avatar, // URL del avatar
    status: data.status
  };
}
...
export async function listMembersServer(projectId) {
  ...
  // Obtener detalles de cada usuario incluyendo avatar
  const membersWithDetails = await Promise.all(
    (data?._embedded?.elements ?? []).map(async (m) => {
      try {
        ...
        console.error(`Error fetching details for user ${m._links?.principal?.title}:`, error.message);
        // Fallback sin detalles adicionales
        return {
          id: m.id,
          user: m._links?.principal?.title,
          roles: (m._links?.roles || []).map(r => r.title),
