...
// Subir avatar y crear/actualizar usuario si es necesario
export async function uploadAvatar(req, res) {
  try {
    ...
    // Datos del usuario desde el body (enviados por el frontend)
    const { openproject_id, name, email, roles } = req.body;
    console.log('User data from body:', { openproject_id, name, email, roles });
    ...
    if (typeof roles === 'string') {
      try {
        ...
        console.log('Error parsing roles JSON, using empty array');
        parsedRoles = [];
      }
    }
    ...
    if (!openproject_id || !name || !email) {
      return res.status(400).json({ message: 'Faltan datos del usuario: openproject_id, name, email' });
    }
    ...
    // Crear o actualizar usuario con avatar
    console.log('Creating/updating user with avatar...');
    const result = await pool.query(
      `INSERT INTO users (openproject_id, name, email, avatar_url, roles, created_at, updated_at)
       ...
       RETURNING *`,
      [openproject_id, name, email, avatarUrl, parsedRoles]
    );

    console.log('User created/updated with avatar:', result.rows[0]);

    res.json({
      message: 'Avatar subido y usuario actualizado correctamente',
      avatarUrl,
      user: result.rows[0]
    });
  } catch (error) {
