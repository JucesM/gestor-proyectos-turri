...
export async function syncUserWithOpenProject(req, res) {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (openproject_id, name, email, roles, last_sync, created_at, updated_at)
...
export async function getUserByOpenProjectId(req, res) {
  try {
    ...
    const result = await pool.query(
      'SELECT id, openproject_id, email, name, roles, created_at, updated_at FROM users WHERE openproject_id = $1',
      [openprojectId]
    );
