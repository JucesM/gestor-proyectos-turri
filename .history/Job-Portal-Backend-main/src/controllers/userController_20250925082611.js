...
export async function uploadAvatar(req, res) {
  try {
    ...
    // Roles ahora es un string simple
    ...
    // Actualizar el usuario logueado con avatar
    console.log('Updating logged in user with avatar...');
    const result = await pool.query(
      `UPDATE users SET
         name = $1,
         email = $2,
       ...
       WHERE id = $5
       RETURNING *`,
      [name, email, avatarUrl, userRole, req.user.id]
    );
...
export async function getUserByIdOrEmail(req, res) {
  try {
    const { identifier } = req.params; // Puede ser ID numérico o email

    let query;
    let param;

    // Verificar si es un número (ID) o email
    if (!isNaN(identifier)) {
