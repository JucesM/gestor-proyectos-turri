...
import {
  ...
  export async function memberships(req, res) {
    try {
      ...
      // Get all projects
      const projects = await listProjectsUser(userToken);
      ...
      for (const project of projects) {
        ...
        let offset = 1;
        let total = Infinity;
        while (wp.length < total) {
          const { data: wpData } = await client.get(`/projects/${project.id}/work_packages?pageSize=100&offset=${offset}`);
          if (wpData._embedded?.elements) {
            wp.push(...wpData._embedded.elements);
          }
          total = wpData.total;
          offset += 100;
        }
        for (const task of wp) {
          const assigneeHref = task._links?.assignee?.href;
          if (assigneeHref) {
            const assigneeId = assigneeHref.split('/').pop();
            if (!userTasks.has(assigneeId)) userTasks.set(assigneeId, new Set());
            userTasks.get(assigneeId).add(project.name);
          }
        }
      }
