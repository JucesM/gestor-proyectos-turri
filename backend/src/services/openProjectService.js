...
export async function listMembersServer(projectId, token) {
  const client = makeClient(token);
  ...
  const membersWithDetails = await Promise.all(
    (data?._embedded?.elements ?? []).map(async (m) => {
      ...
      if (userId) {
        userDetails = await getUserDetails(userId, token);
      }
...
export async function listWorkPackagesServer(projectId, filters = [], token) {
  const client = makeClient(token);
